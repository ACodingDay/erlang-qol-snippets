...
Zed extensions cannot register commands, so module templates are triggered by binding the [`editor::InsertSnippet`](https://zed.dev/docs/keymaps) action in your user keymap (`~/.config/zed/keymap.json` on macOS and Linux, `%APPDATA%\Zed\keymap.json` on Windows):
...
Press `ctrl-shift-p` then the letter. Note that when a chord is pending, pressing `ctrl-shift-p` alone opens the command palette after a ~1s delay. `$1`–`$4` tabstops guide you through module name, author, date and template-specific fields (e.g. record fields or exports).
...
2. Run the merge script (PowerShell 7 `pwsh`, no dependencies):
...
- Do not edit `snippets/erlang.json` directly — it is generated by `build.ps1`. The script requires PowerShell 7 (`pwsh`): Windows PowerShell 5.1 formats `ConvertTo-Json` output differently and would rewrite the whole generated file. The build also fails if the prefix tables or keybinding snippet names in the READMEs drift from the snippets, and CI regenerates everything and fails if it is out of date.
